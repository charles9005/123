...
def get_jql():
    jql = JiraTest(conf_filepath).jql
    return jql
...
class JiraTest:
    ...
    def get_issues(self):
        ...
        issue_lists = self.connect_jira().search_issues(jql, maxResults=500)
        return issue_lists
    ...
    def print_project_info(self):
        project_name = self.connect_jira().project(self.project_key).name
        print("正在下载", self.get_project_info(), "项目的bug汇总信息，请等待...")
    ...
    def create_output_excel(self):
        ...
        wb_name = "Test_status_of_{0}_project_{1}.xlsx".format(self.get_project_info(), nowday)
        wb_name = wb_name.replace(" ", "_")
        wb.save(wb_name)
        wb.close()
        print("即将打开文件，请稍后...")
